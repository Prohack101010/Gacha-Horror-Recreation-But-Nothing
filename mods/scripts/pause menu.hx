...
function onPause()
{
	game.mobileControls.visible = false;
...
var colours = [
	'default' => 0xFF000000, 'pink' => 0xFFFA86C4, 'crimson' => 0xFF870007, 'turquoise' => 0xFF30D5C8, 'red' => 0xFFBB0000, 'green' => 0xFF00AA00,
	'blue' => 0xFF0000BB, 'purple' => 0xFF592693, 'yellow' => 0xFFC8B003, 'brown' => 0xFF664229, 'orange' => 0xFFFFA500, 'custom' => 0xFF000000
];
...
function onCustomSubstateCreate(t)
{
	switch (t)
	{
		case 'pauseMenu':
			...
			{
				var option = new FlxText(0, ((i + 1) * 80) + 75, 0, options[i], 40);
...
function changeSelection(?dir:Int)
{
	...
	if (options.length <= 5)
		...
		for (i in 0...options.length)
			FlxTween.tween(optionObjects.get('option' + i), {y: ((i - (curSelected - 1)) * 80) + 135}, settings.optionTweenTime, {ease: FlxEase.quadOut});
...
function fadeOut()
{
	...
	if (state == null)
	{
		...
		FlxTween.tween(bgGrid, {alpha: 0}, 0.25, {ease: FlxEase.quadOut, onComplete: function()
		{
			game.mobileControls.visible = false;
					...
					case 'exit':
						...
						else
						{
							switch (PlayState.SONG.song)
							{
								...
								case 'Old Despair' | 'Old Isolation' | 'Old Red Slot' | 'Mike' | 'Luniverse' | 'Christmas Horror': MusicBeatState.switchState(new FreeplayStateOld());
