characterPath = 'characters/SarahGhost' -- Picture File name of spreadsheet (must have .xml)
...
antialiasing = getPropertyFromClass('backend.ClientPrefs', 'data.antialiasing') -- If true, it will use antialiasing for the sprite which makes it more detailed.
...
--]]
animationData = {
    ['idle'] = {'SarahGhost Idle0', 0, 0, '', false},
    ['idle-alt'] = {'SarahGhost ALT0', -500, 0, '', true},

    ['singLEFT'] = {'SarahGhost Left0', 0, 3, '', false},
    ['singDOWN'] = {'SarahGhost Down0', 0, -1, '', false},
    ['singUP'] = {'SarahGhost Up0', 0, -1, '', false},
    ['singRIGHT'] = {'SarahGhost Right0', 0, -1, '', false},
}
...
characterCamera = {
    ...
    100 -- Y (Offset) for the camera to be targetting when it is extra character's turn.
}
...
function onUpdate(elapsed)
    ...
    if flipX then 
        ...
        if holdTimer > stepCrochet * (0.0011 / getPropertyFromClass('flixel.FlxG', 'sound.music.pitch')) * singAnimationLength and stringStartsWith(getProperty(characterName .. '.animation.curAnim.name'), 'sing') and not stringEndsWith(getProperty(characterName .. '.animation.curAnim.name'), 'miss') then
            dance()
        end
    end
